...
Phase 1D pins NDK `29.0.14206865` and JNA `5.19.1` in `android/gradle/libs.versions.toml`, and `cargo-ndk` `4.1.2` plus native API 26 in consumed `android/gradle.properties` keys, for the Rust/Android FFI path documented in [ADR 014](../decisions/014-rust-android-uniffi-bridge.md). Rust `1.88.0` is owned by `rust/rust-toolchain.toml`. Native builds target API 26 and package `arm64-v8a` plus `x86_64` only. Gradle rejects substring near-matches when verifying `rustc` and `cargo-ndk`.
