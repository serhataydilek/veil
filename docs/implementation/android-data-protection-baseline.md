...
This is configuration, not a proof of deletion or universal OEM behavior. Before identity or message persistence, validate backup/restore and transfer behavior on supported Android versions/OEMs, re-check Android’s current rules, and add platform-specific storage controls. Phase 1A does not implement Keystore identity creation, biometric lock, screenshot protection, clipboard cleanup, or keyboard-learning controls.
