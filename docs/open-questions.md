...
- **Concrete one-to-one library approval:** the Signal-style protocol direction is selected (ADR 005). Phase 1E records **libsignal v0.99.1** (`97801d22dcf9f5bf714f7b8fa3212cdc973ae1c8`) as the technically preferred candidate and **BLOCKED** (ADR 015). Phase 1K provides encrypted opaque security-record storage, non-destructive migration, local transactional multi-record mutation, SQLite and in-memory rollback parity, and encryption/write-failure rollback coverage. It does not provide an IdentityKeyStore, prekey/session stores, real callback semantics, ratchet crash tests, or atomic composition of session state with message/application state. Legal distribution, unsupported-upstream plan, operational pin/SBOM/advisory process, independent application review, intended libsignal-pin compilation with Rust 1.88, and remaining operational dependency gates must still pass; no custom ratchet is permitted. The crate is not added.
