...
The relay should minimize linkability through short-lived rendezvous state, opaque capability-pair tags, rotating mailbox epochs, narrowly scoped rate-limit buckets, and deletion. It cannot promise to hide source IP, connection timing, traffic volume, a live mailbox-epoch rotation overlap, or the fact that it routes to a mailbox. Padding into a small set of ciphertext size buckets reduces exact length leakage but costs bandwidth and does not hide timing; use only after measurement and review. Delays/cover traffic are not promised.

For V1, the relay holds a short-lived, purpose-limited push-token mapping and sends only a generic wake-up. Default notification text is exactly `New message`; no alias, identity, plaintext, contact ID, mailbox ID, count, preview, or conversation label. This exposes token/timing linkage between relay activity and the push provider. A separate wake-up service is not justified in V1; polling remains a later user-choice evaluation.
