...
Phase 1B's Android local-protection sentinel is not this lifecycle's identity. It may report local storage as ready, unavailable, or unreadable, but it never creates, replaces, or restores a Veil protocol identity. Phase 1C App Lock authenticates local UI access only and is not identity authentication.
