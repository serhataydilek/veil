...
- Phase 1H permits a conservative 30-second monotonic, process-memory-only grace period after real backgrounding when App Lock is enabled. Configuration changes are not treated as backgrounding. Process death, reboot, unavailable state, a negative monotonic duration, and expiry lock immediately. Authentication-in-progress is modeled so the system credential UI does not race-lock the session.
